...
// Inlined VoidButton component to make the component fully self-contained
...
export function VoidButton({
  ...
  return (
    <button
      ...
      className={cn(
        "relative w-full h-12 rounded border border-neutral-900 bg-[#07070a] font-mono text-xs uppercase tracking-widest cursor-pointer select-none overflow-hidden outline-none transition-colors duration-300",
        className
      )}
      style={{
        boxShadow: "inset 0 3px 8px rgba(0,0,0,0.9), inset 0 -1px 2px rgba(255,255,255,0.03), 0 1px 1px rgba(0,0,0,0.4)",
      }}
      {...props}
    >
      <span className="absolute inset-0 flex items-center justify-center text-white/70 font-bold transition-opacity duration-300">
        {children || "THE VOID"}
      </span>
...
export function MechanicalTimer({ className = "" }: { className?: string }) {
  ...
  return (
    <div className={`w-full max-w-[340px] p-6 rounded-3xl bg-[#0a0a0d] border border-white/5 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.9),0_30px_60px_-30px_rgba(0,0,0,0.95),inset_0_1px_1px_rgba(255,255,255,0.03)] flex flex-col items-center gap-6 ${className}`}>
      ...
      {/* Control Buttons Stacked Layout (Spacious and prominent) */}
      <div className="w-full flex flex-col gap-3">
        {/* Row 1: Primary Start / Stop Button (Large, full width) */}
        <VoidButton
