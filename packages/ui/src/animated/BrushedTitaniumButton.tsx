...
/** Titanium machined-metal button with smooth cursor-controlled reflection sweeps and minimal dark metal finish. */
export interface BrushedTitaniumButtonProps extends Omit<React.ComponentPropsWithoutRef<"button">, "style"> {
  length?: number | string;
  width?: number | string;
  style?: React.CSSProperties;
}

export function BrushedTitaniumButton({
  className,
  children,
  length,
  width,
  ...
  const handleMouseLeave = () => {
    setIsHovered(false);
    // Return springs to center for natural resting reflection
    if (containerRef.current) {
      mouseX.set(containerRef.current.offsetWidth / 2);
      mouseY.set(containerRef.current.offsetHeight / 2);
    }
  ...
  // Translate the sweep reflection highlight layer using GPU transform (silky smooth)
  const sweepX = useTransform(springX, (x) => {
    if (!containerRef.current) return 0;
    const btnWidth = containerRef.current.offsetWidth || 192;
    return ((x / btnWidth) * 120) - 60;
  });

  // Dynamic spotlight reflection template
  ...
  return (
    <motion.button
      ...
      className={cn(
        "relative rounded font-syncopate text-[9px] uppercase tracking-[0.25em] font-bold cursor-pointer select-none overflow-hidden outline-none border border-neutral-700/60 bg-neutral-900 transition-shadow duration-300 flex items-center justify-center",
        sizeClass,
        className
      )}
      style={{
      ...
      {...(props as any)}
    >
      {/* Anisotropic sweep reflection highlight (GPU-accelerated horizontal translation) */}
      <motion.div
        ...
        style={{
          background:
            "linear-gradient(115deg, transparent 35%, rgba(255,255,255,0.15) 48%, rgba(255,255,255,0.3) 50%, rgba(255,255,255,0.15) 52%, transparent 65%)",
          x: sweepX,
        }}
      />
