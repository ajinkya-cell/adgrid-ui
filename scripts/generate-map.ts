import { registry } from '../apps/docs/src/registry/index';
...
md += '> **Total Registered Components**: ' + registry.length + '  \n';
md += '> **Generated**: ' + new Date().toISOString().split('T')[0] + '\n\n';
md += '---\n\n';
md += '## 1. Monorepo Architecture & The 5-Point Component Linkage\n\n';
md += 'Whenever editing, debugging, or creating any component in this monorepo, 5 key touchpoints are linked together:\n\n';
...
md += '---\n\n';
md += '## 2. Complete Component Index (' + registry.length + ' Components)\n\n';
md += '| # | Component Name | Slug | Category | Pure Source Path | Key Dependencies |\n';
md += '| :--- | :--- | :--- | :--- | :--- | :--- |\n';

registry.forEach((entry, idx) => {
  const deps = entry.dependencies && entry.dependencies.length > 0
    ? entry.dependencies.map(d => '`' + d + '`').join(', ')
    : 'None';
  const relPath = `packages/ui/src/${entry.packagePath}`;
  const fileUrl = `file:///C:/Users/ajink/OneDrive/Desktop/personal%20-%20coding%20-%20ventures/adgrid-ui/adgrid-ui/${relPath}`;
  md += `| ${idx + 1} | **${entry.name}** | \`${entry.slug}\` | \`${entry.category}\` | [\`${relPath}\`](${fileUrl}) | ${deps} |\n`;
});
...
console.log('Successfully generated COMPONENT_MAP.md with ' + registry.length + ' components.');
